...
class MockZplDataElement extends ZplDataElement {
  const MockZplDataElement(this.numberOfCommands)
    : super(elementData: '<data>', x: 24, y: 50, alignment: ZplAlignment.left);
  ...
  List<ZplCommand> get zplCommands =>
      List.generate(numberOfCommands, (_) => const MockZplCommand(a: 1));

  @override
  List<Object?> get props => super.props..addAll([numberOfCommands]);
}
...
void main() {
  test('`ZplDataElement` core functionality', () {
    ...
    expect(const MockZplDataElement(0).toZpl(), '$expectedStart$expectedEnd');
    expect(
